...
export default function Board() {
  ...
  useEffect(() => {
    ...
    getMembers(activeWorkspace.id).then((r) => setMembers(r.data.members || [])).catch(() => {});
  }, [activeWorkspace?.id]);
  ...
  const handleStatusChange = async (task, newStatus) => {
    ...
    } catch { toastError('Failed to update task status.'); }
  };
  ...
  const handleCreateTask = async (e) => {
    ...
    setCreating(true);
    try {
      const payload = { ...newTask, assignee_id: newTask.assignee_id || undefined, due_date: newTask.due_date || undefined };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="bg-white border-b border-gray-200 px-5 py-3 flex items-center gap-4 flex-shrink-0">
        ...
        <h1 className="text-base font-semibold text-gray-900 flex-1 truncate">Board</h1>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="New task" size="md">
        <form onSubmit={handleCreateTask} className="space-y-4">
          <div>
            ...
            <input autoFocus required value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })}
