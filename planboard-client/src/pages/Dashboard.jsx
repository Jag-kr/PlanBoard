...
export default function Dashboard() {
  ...
  useEffect(() => { fetchStats(); }, [activeWorkspace?.id]);
  ...
  if (!activeWorkspace) return (
    <div className="flex items-center justify-center h-64">
      <EmptyState icon="🏢" title="No workspace" description="Create a workspace to get started." />
    </div>
  );
  ...
  const statCards = [
    { label: 'Total Projects', value: stats?.totalProjects ?? 0,  icon: '📁', color: 'text-blue-600',   bg: 'bg-blue-50' },
    { label: 'Open Tasks',     value: stats?.openTasks ?? 0,      icon: '📝', color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Overdue',        value: stats?.overdueTasks ?? 0,   icon: '⚠️',  color: 'text-red-600',   bg: 'bg-red-50' },
    { label: 'Completed',      value: stats?.completedTasks ?? 0, icon: '✅',  color: 'text-green-600', bg: 'bg-green-50' },
  ];
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => (
          <div key={card.label} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className={`inline-flex h-10 w-10 items-center justify-center rounded-xl ${card.bg} text-xl mb-3`}>
              {card.icon}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h2 className="text-base font-semibold text-gray-800 mb-4">My Tasks</h2>
              ...
              {stats?.myTasks?.map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {task.due_date && (
                      <span className={`text-xs ${isOverdue(task.due_date) ? 'text-red-600 font-semibold' : 'text-gray-400'}`}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {stats?.recentActivity?.length === 0 ? (
            <EmptyState icon="📭" title="No activity yet" description="Start creating tasks to see activity here." />
          ) : (
            <div className="space-y-2">
              {stats?.recentActivity?.map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex-shrink-0 text-right">
                    <StatusBadge status={task.status} />
                    <p className="text-xs text-gray-400 mt-0.5">{timeAgo(task.updatedAt)}</p>
                  </div>
                </div>
              ))}
            </div>
