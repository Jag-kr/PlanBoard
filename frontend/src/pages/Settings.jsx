  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">Manage workspace settings</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            />
            {!isAdmin && <p className="text-xs text-gray-400 mt-1">Only ADMINs can change the workspace name.</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Slug</label>
            <input value={activeWorkspace.slug} readOnly
          ...
          <div>
            ...
            <input value={activeWorkspace.role} readOnly
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm bg-gray-50 text-gray-500 cursor-not-allowed" />
          </div>
          {isAdmin && (
            <div className="flex justify-end">
              <button type="submit" disabled={saving || !name.trim() || name === activeWorkspace.name}
      ...
      {isAdmin && (
        <div className="bg-white rounded-xl shadow-sm border border-red-100 p-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Deleting a workspace permanently removes all projects, tasks, and members. This action cannot be undone.
          </p>
          <button
            disabled
          ...
          <p className="text-xs text-gray-400 mt-2">Contact support to delete a workspace.</p>
        </div>
      )}
    </div>
  );
