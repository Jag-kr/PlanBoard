...
export default function Navbar() {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (wsRef.current && !wsRef.current.contains(e.target))   setWsDropOpen(false);
    ...
    <>
      <nav className="bg-white border-b border-gray-200 h-14 flex items-center px-4 gap-4 sticky top-0 z-30">
        ...
        <div className="flex items-center gap-3 mr-4">
          ...
          <span className="font-bold text-gray-900 text-base hidden sm:block">PlanBoard</span>
            ...
            {wsDropOpen && (
              <div className="absolute left-0 top-full mt-1 w-56 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1 overflow-hidden">
                ...
                {workspaces.map((ws) => (
                  <button
                    key={ws.id}
                    onClick={() => { switchWorkspace(ws); setWsDropOpen(false); navigate('/dashboard'); }}
                    className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 hover:bg-gray-50 transition-colors ${
                      ws.id === activeWorkspace?.id ? 'text-blue-600 font-medium' : 'text-gray-700'
                    }`}
                  >
                    ...
                    {ws.id === activeWorkspace?.id && (
                      <svg className="ml-auto h-3.5 w-3.5 text-blue-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className="border-t border-gray-100 mt-1 pt-1">
                  <button
                    onClick={() => { setWsDropOpen(false); setNewWsModal(true); }}
        ...
        <div ref={userRef} className="relative">
          ...
          {userDropOpen && (
            <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1">
              ...
              <button
                onClick={() => { setUserDropOpen(false); logout(); navigate('/login'); }}
      ...
      <Modal isOpen={newWsModal} onClose={() => setNewWsModal(false)} title="Create new workspace" size="sm">
        <form onSubmit={handleCreateWorkspace} className="space-y-4">
          ...
          <div className="flex gap-3 justify-end">
            <button type="button" onClick={() => setNewWsModal(false)}
