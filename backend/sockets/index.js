  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join:workspace", (workspaceId) => {
      if (typeof workspaceId === "string" && workspaceId.trim()) {
        ...
        console.log(
          `[Socket] ${socket.user?.email} joined workspace:${workspaceId}`,
        );
      }
    });
    ...
    socket.on("leave:workspace", (workspaceId) => {
      if (typeof workspaceId === "string" && workspaceId.trim()) {
        ...
        console.log(
          `[Socket] ${socket.user?.email} left workspace:${workspaceId}`,
        );
      }
    });
    ...
    socket.on("disconnect", (reason) => {
      console.log(
        `[Socket] User disconnected: ${socket.user?.email || socket.id} (${reason})`,
      );
    });
  });
};
