...
const invite = async (req, res, next) => {
  try {
    ...
    const pendingInvite = await Invitation.findOne({
      where: { workspace_id: workspaceId, email: normalizedEmail, status: 'PENDING' }
    });
    if (pendingInvite) {
    ...
    await sendInvitationEmail(normalizedEmail, token, workspace.name, req.user.name);
...
const updateRole = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ member: { id: member.id, userId, role: member.role } });
  } catch (err) {
    next(err);
  }
};
  ...
  try {
    ...
    if (userId === req.user.id) {
      ...
      if (adminCount <= 1) {
        return res
          .status(400)
          .json({ error: 'Cannot remove yourself — you are the last admin of this workspace.' });
      }
    }
...
module.exports = { getMembers, invite, acceptInvitation, updateRole, removeMember };
